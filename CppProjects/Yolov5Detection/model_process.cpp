...
Result ModelProcess::LoadModelFromFileWithMem(const char *modelPath)
{
    ...
    // if (ret != ACL_ERROR_NONE)
    // {
    //     ERROR_LOG("Get aipp info failed, error code: [%d]", ret);
    // }
    ...
    //     INFO_LOG("tensor name: %s, shape: [%ld, %ld, %ld, %ld]", io_dims.name, io_dims.dims[0], io_dims.dims[1], io_dims.dims[2], io_dims.dims[3]);
    // }
