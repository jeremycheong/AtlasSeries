...
Result ModelProcess::LoadModelFromFileWithMem(const char *modelPath)
{
    ...
    aclAippInfo model_aipp_info;
    ret = aclmdlGetFirstAippInfo(modelId_, 0, &model_aipp_info);
    modelInputWidth_ = model_aipp_info.srcImageSizeW;
    modelInputHeight_ = model_aipp_info.srcImageSizeH;
    aclAippInputFormat aipp_input_format = model_aipp_info.inputFormat;
    INFO_LOG("input format: %d", aipp_input_format);
    size_t shape_cnt =  model_aipp_info.shapeCount;
    for (size_t i = 0; i < shape_cnt; i ++)
    {
        aclAippDims aipp_dims = model_aipp_info.outDims[i];
        aclmdlIODims io_dims = aipp_dims.srcDims;

        INFO_LOG("tensor name: %s, shape: [%d, %d, %d, %d]", io_dims.name, io_dims.dims[0], io_dims.dims[1], io_dims.dims[2], io_dims.dims[3]);
    }
...
Result ModelProcess::CreateInput(std::vector<void *> inputBuffers, std::vector<size_t> inputBufferSize)
{
    ...
    for (size_t i = 0; i < inputBufferSize.size(); i++)
    {
        auto &input_buffer = inputBuffers[i];
        auto &input_buffer_size = inputBufferSize[i];
        aclDataBuffer *inputData = aclCreateDataBuffer(input_buffer, input_buffer_size);
...
aclmdlDataset *ModelProcess::GetModelOutputData()
{
 ...
 const uint32_t ModelProcess::GetModelWidth()
 {
     return modelInputWidth_;
 }

 const uint32_t ModelProcess::GetModelHeight()
 {
     return modelInputHeight_;
 }
