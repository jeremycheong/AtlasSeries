...
//     #define ERROR_LOG(fmt, args...) fprintf(stdout, "[ERROR]  [%s:%d] %s: " fmt "\n", __FILE__, __LINE__, __FUNCTION__, ##args)
// }
...
public:
    ...
    static std::vector<std::string> getfiles(const std::string &path_str, const std::vector<std::string> &text_words);
...
std::vector<std::string> Path::getfiles(const std::string &path_str, const std::vector<std::string> &text_words)
{
    std::vector<std::string> results;
    for (const auto &text_word : text_words)
    {
        auto results_sig = getfiles(path_str, text_word);
        results.insert(results.end(), results_sig.begin(), results_sig.end());
    }
    return results;
}
